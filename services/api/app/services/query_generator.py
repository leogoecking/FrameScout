...
STOP_WORDS = {
    ...
    "com", "sem", "sobre", "entre", "que", "se", "e", "ou", "mas", "como", "mais",
    "foi", "foram", "era", "eram", "sao", "são", "ser", "ter", "está", "estao", "estão",
...
class QueryGenerator:
    ...
    def generate(
        ...
        # Extrair entidades com letras maiúsculas (ex: "Take-Two", "CrowdStrike", "GTA VI")
        proper_nouns = re.findall(
            r"\b[A-Z][a-zA-Z0-9]*(?:[-][A-Z][a-zA-Z0-9]*)*(?:\s+[A-Z][a-zA-Z0-9]*)*\b",
            narration,
        )
        proper_nouns = [p for p in proper_nouns if p.lower() not in STOP_WORDS and len(p) > 2]
        ...
        if event_query_str and event_query_str.lower() not in seen_queries:
        ...
        if not companies and proper_nouns:
            official_q = clean_query_term(f"{proper_nouns[0]} official")
            if official_q.lower() not in seen_queries:
                seen_queries.add(official_q.lower())
                queries.append(
                    SearchQueryCreate(
        ...
        for b_q in broll_terms:
            if b_q and b_q.lower() not in seen_queries:
        ...
        # Garantir pelo menos uma query de B-roll se o conjunto estiver pequeno
        if len(queries) < 2 and keywords:
            fallback_broll = clean_query_term(f"{' '.join(keywords[:3])} broll")
            if fallback_broll.lower() not in seen_queries:
                queries.append(
                    SearchQueryCreate(
                        query=fallback_broll,
                        query_type=QueryType.BROLL,
                        priority=2,
                    )
                )
